...
// Auto-detect the project root: works whether files sit at the repo root
// or one level down (e.g. inside a "gno-cms" folder from the zip).
let BASE = '.';
const ROOT = process.cwd();
if (!fs.existsSync('content')) {
  const cand = fs.readdirSync('.').find(d => {
    try { return fs.statSync(d).isDirectory() && fs.existsSync(path.join(d, 'content')); }
    catch (e) { return false; }
  });
  if (cand) { BASE = cand; process.chdir(BASE); console.log('Building from folder:', BASE); }
  else { console.error('ERROR: cannot find the content folder. Make sure the folders admin, assets, content, templates were uploaded.'); process.exit(1); }
}
